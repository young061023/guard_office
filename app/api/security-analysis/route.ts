...
export async function POST(request: Request) {
  ...
  try {
    ...
    let model = configuredModel==="gemini-2.5-flash" ? "gemini-3.5-flash" : configuredModel;
...
${previousSerialized}
    ...
    // Retry transient provider failures, never authentication errors or daily quotas.
    for(let attempt=0; attempt<3 && [500,502,503,504].includes(geminiResponse.status); attempt++){
      await geminiResponse.body?.cancel();
      await new Promise(resolve=>setTimeout(resolve,1500 * 2**attempt));
      geminiResponse=await requestModel(model);
    }
    if([500,502,503,504].includes(geminiResponse.status) && model!=="gemini-3.1-flash-lite"){
      await geminiResponse.body?.cancel();
    ...
    if (!geminiResponse.ok) {
      ...
      console.error("Gemini API error", {status:geminiResponse.status,model});
      ...
      const error=status===429
        ...
        : status===503?"Gemini 서버 혼잡으로 재시도 후에도 분석하지 못했습니다. 잠시 후 다시 검사해주세요."
        : "Gemini 분석 요청이 실패했습니다. 잠시 후 다시 시도해주세요.";
      return Response.json({ error }, { status: status===429?429:status===503?503:502 });
    }
