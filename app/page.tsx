...
export default function Home(){
  ...
  const loadSample=async(name:string,mode:"logs"|"sql")=>{
    setError("");
    try{
      const response=await fetch(`/samples/${name}`);
      if(!response.ok)throw new Error("sample unavailable");
      const content=await response.text();
      const type=mode==="sql"?"application/sql":"text/csv";
      await readFile(new File([content],name,{type}));
    }catch{
      setError("테스트 파일을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.");
    }
  };
