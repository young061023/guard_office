...
export default function Home(){
  ...
  const readFile=async(file?:File)=>{
    ...
    try{
      ...
      setFileInfo(pendingInfo);setSources(current=>({...current,[sourceKind]:pendingInfo}));setSelectedId(isSql?"data":"soc");setUploadOpen(false);setActiveCount(0);setMonitoring(true);setDepartmentLoading({kind:sourceKind,phase:"splitting"});
  ...
  const issueOverview=fileInfo?<section className={`security-issues-window inline-window ${fileInfo.kind}`} role="dialog" aria-modal="true" aria-labelledby="issues-title">
    ...
    <section className={`reference-stage ${fileInfo?.mode==="analyzing"?"is-analyzing":""}`}>
      <div className="company-canvas">
        <div className="company-artboard">
          ...
          <div className="reference-rooms">
            {rooms.map(room=>{const dbReady=room.id==="data";const connected=dbReady?Boolean(sources.sql):Boolean(sources.logs);const loading=departmentLoading?.kind===(dbReady?"sql":"logs");return <button key={room.id} className={`reference-room ${room.cls} ${connected?"is-connected":"is-disconnected"} ${loading?"is-loading":""} ${room.problem?"has-problem":""}`} onClick={()=>openRoom(room.id)} aria-label={`${room.name} 열기`}>
              <RoomIcon id={room.id}/>
              <strong>{roomDisplayNames[room.id]??room.name}</strong>{loading?<span className={`room-loading ${departmentLoading?.phase??"loading"}`} aria-label="분석 중"><i/><i/><i/></span>:<i aria-hidden="true"/>}<ChevronRight className="room-action-arrow" aria-hidden="true"/>{room.problem&&<span className="room-alert-glow" aria-hidden="true"/>}
            </button>})}
          </div>
      ...
      <nav className="connection-actions" aria-label="보안 자료 연결">
        ...
        {overviewOpen&&fileInfo?.kind==="sql"?null:dbResult
          ?<button ref={dbConnectRef} className="analysis-result-button db-result" onClick={()=>{setFileInfo(dbResult);setOverviewOpen(true)}}><Database aria-hidden="true"/><strong>분석 결과 보기</strong><ChevronRight aria-hidden="true"/></button>
    ...
    {dashboardOpen&&fileInfo&&<div className="modal-backdrop dashboard-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setDashboardOpen(false)}}>
      <div className="pixel-report-scene">
        <aside className={`report-character-slot ${selectedCharacter?"has-character":"empty"}`} aria-label={`${selected.name} 담당 캐릭터`}>
          {selectedCharacter?<img src={selected.problem?selectedCharacter.surprised:selectedCharacter.idle} alt={`${selected.name} 담당 캐릭터`}/>:<span>AGENT</span>}
        </aside>
