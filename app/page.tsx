...
import { AppWindow, BadgeCheck, ChevronRight, CircleAlert, CloudCog, Database, FolderOpen, ScanSearch, ShieldAlert, Sparkles, TriangleAlert } from "lucide-react";
  ...
  const readFile=async(file?:File)=>{
    ...
    try{
      ...
      try{
        ...
        await waitForMotion();console.warn(aiError);const fallbackInfo:FileInfo={name:file.name,records:recordCount,types:detectedTypes,kind:sourceKind,mode:"rules",summary:"Gemini 연결을 사용할 수 없어 기본 규칙으로 분석했습니다."};setFileInfo(fallbackInfo);setSources(current=>({...current,[sourceKind]:fallbackInfo}));
        setSourceContents(current=>isSql?{...current,sql:text}:{...current,logs:records});
      }
      ...
      <div className="company-canvas">
        <div className="company-artboard">
          ...
          <div className="reference-rooms">
            {rooms.map(room=>{const dbReady=room.id==="data";const connected=dbReady?Boolean(sources.sql):Boolean(sources.logs);return <button key={room.id} className={`reference-room ${room.cls} ${connected?"connected":""} ${room.problem?"has-problem":""}`} onClick={()=>openRoom(room.id)} aria-label={`${room.name} 열기`}>
              <RoomIcon id={room.id}/>
              <strong>{roomDisplayNames[room.id]??room.name}</strong><i aria-hidden="true"/><ChevronRight className="room-action-arrow" aria-hidden="true"/>
            </button>})}
          </div>
        </div>
      </div>
      <nav className="connection-actions" aria-label="보안 자료 연결">
        <button className="log-connect" onClick={()=>openUpload("logs")}><FolderOpen className="action-icon" aria-hidden="true"/><strong>보안 로그 연결</strong><ChevronRight className="action-arrow" aria-hidden="true"/></button>
        <button className="db-connect" onClick={()=>openUpload("sql")}><Database className="action-icon" aria-hidden="true"/><strong>DB 보안 연결</strong><ChevronRight className="action-arrow" aria-hidden="true"/></button>
      </nav>
      {fileInfo?.mode==="analyzing"&&<div className="analysis-loading" role="status" aria-live="polite"><span className="dot-spinner" aria-hidden="true">{Array.from({length:8},(_,index)=><i key={index} style={{"--dot-index":index} as CSSProperties}/>)}</span><strong>AI 분석 중</strong></div>}
      {fileInfo&&fileInfo.mode!=="analyzing"&&<button className="analysis-result-button" onClick={()=>setOverviewOpen(true)}><FolderOpen aria-hidden="true"/><strong>분석 결과 보기</strong><ChevronRight aria-hidden="true"/></button>}
    </section>
    {overviewOpen&&fileInfo&&<div className="overview-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setOverviewOpen(false)}}>
      <section className="security-issues-window" role="dialog" aria-modal="true" aria-labelledby="issues-title">
        ...
        <div className="issues-body"><h2 id="issues-title">최근 보안 이슈</h2>
          <article className="ai-result-card"><Sparkles aria-hidden="true"/><div><strong>AI 분석 결과</strong><p>{fileInfo.summary||`${fileInfo.name}에서 ${fileInfo.records.toLocaleString()}개 기록을 분석했습니다. 각 부서별 보안 상태와 권장 조치를 확인하세요.`}</p></div></article>
          <div className="processed-count"><strong>처리 기록 : {fileInfo.records.toLocaleString()}건</strong></div>
          ...
          <div className="issues-actions"><button onClick={()=>{setOverviewOpen(false);openUpload("logs")}}>새 로그 검사 <ChevronRight aria-hidden="true"/></button><button onClick={()=>{setOverviewOpen(false);if(logComparison?.hasBaseline)setLogComparisonOpen(true);else openUpload("logs")}}>로그 재검사 <ChevronRight aria-hidden="true"/></button></div>
        </div>
      </section>
    </div>}
